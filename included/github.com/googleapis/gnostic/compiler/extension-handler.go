...
import (
	...
	"github.com/aaron-prindle/krmapiserver/included/github.com/golang/protobuf/ptypes/any"
	ext_plugin "github.com/aaron-prindle/krmapiserver/included/github.com/googleapis/gnostic/extensions"
	yaml "github.com/aaron-prindle/krmapiserver/included/gopkg.in/yaml.v2"
)
...
func (extensionHandlers *ExtensionHandler) handle(in interface{}, extensionName string) (*any.Any, error) {
	if extensionHandlers.Name != "" {
		...
		if len(response.Error) != 0 {
			message := fmt.Sprintf("Errors when parsing: %+v for field %s by included extension handler %s. Details %+v", in, extensionName, extensionHandlers.Name, strings.Join(response.Error, ","))
